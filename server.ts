...
app.post("/api/ds1/read-document", async (req, res) => {
  try {
    ...
    if (extension === ".txt") {
      ...
      (workbook as any).eachSheet((worksheet: any) => {
      (worksheet as any).eachRow((row: any) => {
...
app.post("/api/vision/ocr", async (req, res) => {
   const { 
    ...
    rawTextSample: reqRawTextSample 
  } = req.body;
  const filename = reqFilename || "BL_SCAN.PDF";
  const rawTextSample = reqRawTextSample || "";
  const ai = getGeminiClient();
